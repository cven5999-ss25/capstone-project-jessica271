...
clean_country_corridor_data |>
  ...
  knitr::kable(digits = 1, caption = "Table 1: Countries by Total USD Funded") # Display a clean table of the data summary. This table presents the summary data, that is, countries, number of corridors, average length, total money used, latest launch year and total money funded. The table is arranged in the order of the most money funded to the least. 
...
ggplot(summary_individual_country_data,
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) # This scatter plot shows the number of corridors per country on the y-axis, with countries on the x-axis. Each point's size and color intensity represent the average corridor length in kilometers, larger and darker points indicate longer corridors.
...
summary_individual_country_data |>
  ...
  labs(title = "Figure 5: Top 10 Countries by Total USD Funded",
       ...
       y = "Total USD Funded",
       color = "Country") +
  theme_minimal() # filter to the first 10 countries with the highest amount funded for a clearer reading.  


summary_individual_country_data |>
  select(country, n_corridors, latest_launch) |>
  arrange(desc(latest_launch)) |>
  knitr::kable(digits = 0, caption = "Table 3: Country Corridors by Latest Launch Year") # Table showing the country corridors in a descending order of the launch years.
